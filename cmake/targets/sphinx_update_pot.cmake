...
if (EXISTS "${PROJ_OUT_REPO_DIR}/scripts/sphinx-pre-install")
    ...
    if (POSITION EQUAL -1)
        ...
        remove_cmake_message_indent()
        message("")
        message("Added 'gettextdocs' target in 'Documentation/Makefile' file.")
        ...
        remove_cmake_message_indent()
        message("")
        message("No need to add 'gettextdocs' target in 'Documentation/Makefile' file.")
        message("File: ${DOCS_MAKEFILE_FILE}")
        message("")
        restore_cmake_message_indent()
    endif()
elseif (EXISTS "${PROJ_OUT_REPO_DIR}/tools/docs/sphinx-pre-install")
    message(STATUS "Adding 'gettextdocs' target in 'Documentation/Makefile' file...")
    file(READ "${DOCS_MAKEFILE_FILE}" DOCS_MAKEFILE_CNT)
    string(FIND "${DOCS_MAKEFILE_CNT}" "gettextdocs:" POSITION)
    if (POSITION EQUAL -1)
        string(APPEND GETTEXTDOCS_CNT "\n")
        string(APPEND GETTEXTDOCS_CNT "gettextdocs:\n")
        string(APPEND GETTEXTDOCS_CNT "\t$(Q)PYTHONPYCACHEPREFIX=\"$(PYTHONPYCACHEPREFIX)\" \\\n")
        string(APPEND GETTEXTDOCS_CNT "\t\t$(srctree)/tools/docs/sphinx-pre-install --version-check\n")
        string(APPEND GETTEXTDOCS_CNT "\t+$(Q)PYTHONPYCACHEPREFIX=\"$(PYTHONPYCACHEPREFIX)\" \\\n")
        string(APPEND GETTEXTDOCS_CNT "\t\t$(PYTHON3) $(BUILD_WRAPPER) $@ \\\n")
        string(APPEND GETTEXTDOCS_CNT "\t\t--sphinxdirs=\"$(SPHINXDIRS)\" $(RUSTDOC) \\\n")
        string(APPEND GETTEXTDOCS_CNT "\t\t--builddir=\"$(BUILDDIR)\" --deny-vf=$(FONTS_CONF_DENY_VF) \\\n")
        string(APPEND GETTEXTDOCS_CNT "\t\t--theme=$(DOCS_THEME) --css=$(DOCS_CSS) --paper=$(PAPER)\n")
        string(APPEND DOCS_MAKEFILE_CNT "${GETTEXTDOCS_CNT}")
        file(WRITE "${DOCS_MAKEFILE_FILE}" "${DOCS_MAKEFILE_CNT}")
        remove_cmake_message_indent()
        message("")
...
if (EXISTS "${DOCS_MEDIA_MAKEFILE_FILE}")
...
set(TRANSLATIONS_REGEX    "['\"]translations['\"]")
string(REGEX MATCH "${EXTENSIONS_LIST_REGEX}" OLD_EXTENSIONS_LIST "${DOCS_CONF_PY_CNT}")
if (OLD_EXTENSIONS_LIST AND
    OLD_EXTENSIONS_LIST MATCHES "${TRANSLATIONS_REGEX}")
    string(REGEX REPLACE "${CMAKE_MATCH_0},[ ]*" "" NEW_EXTENSIONS_LIST "${OLD_EXTENSIONS_LIST}")
    string(REGEX REPLACE ",[ ]*${CMAKE_MATCH_0}" "" NEW_EXTENSIONS_LIST "${NEW_EXTENSIONS_LIST}")
