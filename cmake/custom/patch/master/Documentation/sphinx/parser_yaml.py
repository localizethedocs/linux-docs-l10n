...
class YamlParser(Parser):
    ...
    def parse(self, inputstring, document):
        ...
        # [patch]
        # During translated builds, Sphinx may append source metadata to
        # document.current_source (e.g. '/path/file.yaml:123:<translated>').
        # Strip that suffix so YAML parsing always gets a real filesystem path.
        if not os.path.exists(fname):
            match = re.search(r":[0-9]+:<[^>]+>$", fname)
            if match:
                candidate = fname[:match.start()]
